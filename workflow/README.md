...
- `backtest/score/pipeline/go` now run with conservative rescue guardrails by default (including market-net commission settlement in backtests); disable with `--no-rescue-guards` when running raw experiments.
